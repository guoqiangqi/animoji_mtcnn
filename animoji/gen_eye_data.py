...
def gen_eye_bbox_data(neg_file, pos_file, save_dir, debug=False):
    ...
    for pos in pos_list:
        pos = pos.strip().split(' ')
        box = list(map(int, pos[1:9]))
        box = np.asarray(box).reshape(2,4)
        for _x1, _y1, _x2, _y2 in box:
            w = _x2 - _x1 + 1
            h = _y2 - _y1 + 1
    ...
    # wmin, wmax, hmin, hmax = 10, 1350, 11, 758
    ...
    for pos in pos_list:
        pos = pos.strip().split(' ')
        im_path = pos[0]
        eye_box = list(map(int, pos[1:9]))
        bbox = list(map(int, pos[9:13]))
        ...
        lx1, ly1, lx2, ly2 = eye_box[:4]
        rx1, ry1, rx2, ry2 = eye_box[4:]
        lw = lx2 - lx1 + 1
        lh = ly2 - ly1 + 1
        rw = rx2 - rx1 + 1
        rh = ry2 - ry1 + 1

        box = np.asarray(eye_box)
        ...
        while (i < 50 or num_pos < 5):
            i += 1
            if i > 100:
                cropped_im = img[ly1:ly2, lx1:lx2, :]
            ...
            size = np.random.randint(int(max(lw, lh) * 0.8), np.ceil(1.25 * max(lw, lh)))
            delta_x = np.random.randint(-lw * 0.2, lw * 0.2)
            delta_y = np.random.randint(-lh * 0.2, lh * 0.2)
            nx1 = int(max(lx1 + lw / 2 + delta_x - size / 2, x1))
            ny1 = int(max(ly1 + lh / 2 + delta_y - size / 2, y1))
            nx2 = min(nx1 + size, x2)
            ny2 = min(ny1 + size, y2)
...
if __name__ == '__main__':
    ...
    pos_list_file = 'images/rotated_image/eyeRectangelList.txt'
    save_dir = 'images/train_image/eye'
    if os.path.exists(save_dir):
        shutil.rmtree(save_dir)
    files = gen_eye_bbox_data(neg_list_file, pos_list_file, save_dir, debug=True)