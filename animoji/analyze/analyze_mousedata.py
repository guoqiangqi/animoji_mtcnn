...
for filepath, (filename,npz)  in out_result.items():
    ...
    lx = landmark_right_mouse[0] - landmark_left_mouse[0] + 1
    ly = landmark_left_mouse[1] - landmark_left_eye[1] + 1
    # print(lx, ly)
    assert(lx >0 and ly > 0), 'lx: {}, ly: {}'.format(lx, ly)
    
    p1 = np.min(mouse_point,axis=0)
    p2 = np.max(mouse_point,axis=0)
    
    landmark_mouse = landmark_pred_rotated[3:]
    p3 = np.min(landmark_mouse,axis=0)
    p4 = np.max(landmark_mouse,axis=0)
